...
Element.prototype.description = function (definitions) {
  ...
  if (name === 'pattern') {
    return '(pattern' + this[this.valueKey] + ')';
  }
  return name;
};
...
RestrictionElement.prototype.description = function (definitions, xmlns) {
  ...
  var restriction = this.children.map(function (child) {
    return child.description();
  }).join("|");
  if (this.children.length > 1) restriction = '(enum^(' + restriction + ')$)';
  ...
  SequenceElement.prototype.description = function (definitions, xmlns) {
    ...
    for (var i = 0, child; child = children[i]; i++) {
      ...
      if (child.name === 'choice') {
        if (!sequence.__exRequired) sequence.__exRequired = [];
        sequence.__exRequired.push({
          minOccurs: child.$minOccurs,
          choices: Object.keys(description)
        });
      } else if (child.$minOccurs > 0 || child.$minOccurs === undefined) {
        if (!sequence.__required) sequence.__required = [];
        sequence.__required.push(child.$name);
      }
    }
    return sequence;
  };
...
WSDL.prototype.xmlToObject = function (xml, callback) {
  ...
  p.onopentag = function (node) {
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        ...
        if (!portType.methods[name]) {
          ...
          throw new errors.UnknownSOAPOperationError(name);
        }
        if (isInput) {
    ...
    var schema = (xsiTypeSchema || (topSchema && topSchema[name]));
    if(!self.options.wsdl_options || !self.options.wsdl_options.ignoreSchemaValidation) validateSchema(schema, element, top.name);
  ...
  p.onclosetag = function (nsName) {
    ...
    validateRequirements(cur.schema, obj, nsName);
  ...
  p.ontext = function (text) {
    ...
    var top = stack[stack.length - 1];
    var value = validateAndParse(top, text);
...
function validateAndParse(top, text) {
  var schema = top.schema;
  var element = top.name;
  var qName = splitQName(schema);
  var name = qName.name;
  var prefix = qName.prefix;
  var RESTRICTION_REGEX = /\((enum|pattern)(.*)\)$/;
  var match = RESTRICTION_REGEX.exec(schema);

  if (match && match.length === 3) {
    name = name.split('|')[0];
    validateRestriction(match, text, prefix, element);
  }

  if (name === 'int' || name === 'integer') {
    return validateAndParseInt(text, element);
  }
  if (name === 'bool' || name === 'boolean') {
    return validateAndParseBoolean(text, element);
  }
  if (name === 'dateTime' || name === 'date') {
    return new Date(text); // todo
  }
  ...
  return top.object + text;
}
function validateSchema(schema, element, parent) {
  if (!schema) {
    throw new errors.SOAPUnexpectedElement(element, parent);
  }
}
function validateRequirements(schema, object, parentName) {
  if (schema && schema.__required) {
    schema.__required.forEach(function (key) {
      if (!(key in object)) {
        throw new errors.SOAPMissingElement(key, parentName);
      }
    })
  }
  if (schema && schema.__exRequired) {
    schema.__exRequired.forEach(function (exRequired) {
      var count = 0;
      exRequired.choices.forEach(function (key) {
        if (key in object) count++;
      });
      if (count === 0 && exRequired.minOccurs > 0) {
        throw new errors.SOAPMissingChoiceError(exRequired.choices, parentName);
      }
      if (count > 1) {
        throw new errors.SOAPChoiceError(exRequired.choices, parentName);
      }
    })
  }
}
function validateRestriction(match, text, type, element) {
  var restriction = match[1];
  var regex = new RegExp(match[2]);
  if (!regex.test(text)) {
    throw new errors.SOAPValidationError(element, text, type, restriction, regex);
  }
}
function validateAndParseInt(text, element) {
  var REGEX = /^\d+$/;

  if (!REGEX.test(text)) {
    throw new errors.SOAPValidationError(element, text, 'int');
  }
  return parseInt(text, 10);
}
function validateAndParseBoolean(text, element) {
  var REGEX = /^(true|false|0|1)$/;
  var lowerText = text.toLowerCase();

  if (!REGEX.test(lowerText)) {
    throw new errors.SOAPValidationError(element, text, 'boolean');
  }
  return lowerText === 'true' || text === '1';
}
