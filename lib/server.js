...
var CURRENT_URL_PLACEHOLDER = '{{CURRENT_URL}}';
...
function replaceWithCurrentUrl(req, str, forceHTTPS) {

  var regex = new RegExp(CURRENT_URL_PLACEHOLDER, 'g');
  ...
  return str.replace(regex, protocol + '://' + req.get('host') + req.baseUrl);
}
...
var Server = function (server, path, services, wsdl, options) {
  ...
  if (typeof server.use === 'function') {
    ...
    server.use(path, function (req, res, next) {
      ...
      function processRequest() {
        ...
        self._requestListener(req, res, next);
      }
    });
  } else {

    throw new Error('This version of node-soap is currently only available for express.js');
  }
  ...
  wsdl.onReady(function (err) {
    ...
    isWsdlReady = true;
    wsdlReadyError = err;
    wsdlReadyListeners.forEach(function (listener) {
...
Server.prototype._processRequestXml = function (req, res, xml, next) {
  var self = this;
  var result;
  var error;
  try {
    ...
    self._process(xml, req, function (result, statusCode) {
      ...
      res.write(replaceWithCurrentUrl(req, result, self.forceHTTPS));
      res.end();
      if (typeof self.log === 'function') {
...
Server.prototype._requestListener = function (req, res, next) {
  var self = this;
  ...
  if (req.method === 'GET') {
    if (reqQuery && reqQuery.toLowerCase() === '?wsdl') {
      ...
      res.write(replaceWithCurrentUrl(req, self.wsdl.toXML(), self.forceHTTPS));
    }
    res.end();
    ...
    if (req.body) {
      return self._processRequestXml(req, res, req.body.toString(), next);
    }
    ...
    req.on('end', function () {
      ...
      self._processRequestXml(req, res, xml, next);
    });
  }
  else {
    res.end();
...
Server.prototype._executeMethod = function (options, req, callback, includeTimestamp) {
  ...
  try {
    var port = this.services[serviceName][portName];
    method = port[methodName].bind(port);
  } catch (error) {
...
exports.CURRENT_URL_PLACEHOLDER = CURRENT_URL_PLACEHOLDER;
